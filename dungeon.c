    ...
    if ((in=map[m].it)) {
        if (it[in].flags&IF_PLAYERBODY) {
            ...
            if (!drop_item(in,250,245) &&
                ...
                !drop_item(in,250,230)) { destroy_item(in); } else set_expire_body(in,PLAYER_BODY_DECAY_TIME);
...
int create_maze(int base,int level,int do_fake,int keys,const int *warrior,const int *mage,const int *seyan,int teleport,int cnr,int show) {
    ...
    while (m>0) {
        ...
        if (!cell[n].l && n%xsize>0 && cell[n-1].special==m-1) { if (m==tmp) { fake=n; fakedir=1; }
            n=n-1; } else if (!cell[n].t && n/xsize>0 && cell[n-xsize].special==m-1) { if (m==tmp) { fake=n; fakedir=2; }
            n=n-xsize; } else if (n%xsize<xsize-1 && !cell[n+1].l && cell[n+1].special==m-1) { n=n+1; if (m==tmp) { fake=n; fakedir=1; }
