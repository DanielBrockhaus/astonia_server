...
void lab2_herald_driver(int cn,int ret,int lastact) {
    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_TEXT) {
            ...
            if (strcasestr(str,"ARATHAS")) { ppd->herald_talkstep=10;  dat->next_talk=ticker+TICKS/2; } else if (strcasestr(str,"ELIAS")) { ppd->herald_talkstep=20; dat->next_talk=ticker+TICKS/2; } else if (strcasestr(str,"FAMILY") && strcasestr(str,"VAULT")) { ppd->herald_talkstep=30; dat->next_talk=ticker+TICKS/2; } else if (strcasestr(str,"ADMINISTRATIVE") && strcasestr(str,"BUILDING")) { ppd->herald_talkstep=40; dat->next_talk=ticker+TICKS/2; } else if (strcasestr(str,"DIARY")) { ppd->herald_talkstep=50; dat->next_talk=ticker+TICKS/2; } else if (strcasestr(str,"REPEAT")) { say(cn,"I will repeat, %s",ch[co].name); ppd->herald_talkstep=0; dat->next_talk=ticker+TICKS; }
        }
    ...
    if (val==0) {
        if (map[it[undead_dat->graveit].x+it[undead_dat->graveit].y*MAXMAP].flags&MF_NOMAGIC) {
            ...
            if (player_dat->numcrypt==max_crypt-1) {
                ...
                if (ppd && ppd->timesgotcryptgold==0) { log_char(co,LOG_SYSTEM,0,"Thou notice a silent jingling as thine enemy hits the ground."); ppd->timesgotcryptgold++; gold=500; } else if (ppd && ppd->timesgotcryptgold==1) { log_char(co,LOG_SYSTEM,0,"Again thou notice a silent jingling as thine enemy hits the ground."); ppd->timesgotcryptgold++; gold=500; } else log_char(co,LOG_SYSTEM,0,"Thou notice nothing special as thine enemy hits the ground.");
            }
        } else {
            // yard grave
            player_dat->numyard++;
            ...
            if (player_dat->numyard==max_yard-1) {
                ppd=set_data(co,DRD_LAB_PPD,sizeof(struct lab_ppd));
                if (ppd && ppd->timesgotyardgold==0) { log_char(co,LOG_SYSTEM,0,"Thou notice a loud jingling as thine enemy hits the ground."); ppd->timesgotyardgold++; gold=7500; } else if (ppd && ppd->timesgotyardgold==1) { log_char(co,LOG_SYSTEM,0,"Again thou notice a loud jingling as thine enemy hits the ground."); ppd->timesgotyardgold++; gold=7500; } else log_char(co,LOG_SYSTEM,0,"Thou notice nothing special as thine enemy hits the ground.");
            }
        }
