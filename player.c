...
//#define CCYCLES
    ...
    for (y=0; y<=DIST*2; y++) {
        ...
        for (x=xs; x<=xe; x++) {
            ...
            if ((ch[cn].flags&CF_INFRAVISION) && light<4) { light=max(4,light); flags=CMF_INFRA; } else if (ch[cn].flags&CF_INFRARED) { light=max(32,light); flags=0; } else flags=0;

            if (light<1) {
                //if (map[m].ch==cn) light=1;	// make player visible to himself all the time
                if (abs(x-DIST)<2 && abs(y-DIST)<2) light=1;
                else if (ch[cn].prof[P_DARK]>=30) { light=1; dark=1; } else if (ch[cn].prof[P_LIGHT]>=30) { light=1; dark=2; } else { flags=0; goto skip; }
            }
