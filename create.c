...
static char* get_text(char *name,char *value,char *ptr) {
    ...
    *name=0;
    ...
    if (*ptr!='=') { value[0]=0; return NULL; } else ptr++;
...
static int add_item(struct it_temp *itt) {
    ...
    for (n=1; n<MAXTITEM; n++) if (!it_temp[n].name[0]) break;
...
int lookup_item(char *name) {
    ...
    for (n=1; n<MAXTITEM; n++) if (!strcmp(it_temp[n].name,name)) return n;
    return 0;
}
...
static int add_char(struct ch_temp *cht) {
    ...
    for (n=1; n<MAXTCHARS; n++) if (!ch_temp[n].name[0]) break;
...
int lookup_char(char *name) {
    ...
    for (n=1; n<MAXTCHARS; n++) if (!strcmp(ch_temp[n].name,name)) return n;
    return 0;
}
...
int destroy_items(int cn) {
    ...
    for (n=0; n<INVENTORYSIZE; n++) if ((in=ch[cn].item[n])) {
            remove_item_char(in);
            destroy_item(in);
        }
