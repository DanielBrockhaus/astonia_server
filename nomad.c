...
void nomad_parse(int cn,struct nomad_data *dat) {
    ...
    for (ptr=nextnv(ch[cn].arg,name,value); ptr; ptr=nextnv(ptr,name,value)) {
        if (!strcmp(name,"nr")) { dat->nr=atoi(value); } else if (!strcmp(name,"diceskill")) { dat->dice_skill=atoi(value); } else if (!strcmp(name,"minbet")) { dat->min_bet=atoi(value); } else if (!strcmp(name,"maxbet")) { dat->max_bet=atoi(value); } else if (!strcmp(name,"maxloss")) { dat->max_loss=atoi(value); } else elog("unknown arg for %s (%d): %s",ch[cn].name,cn,name);
    }
}
