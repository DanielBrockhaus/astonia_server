    ...
    if (ppd->points>=ppd->base/4) {
        ...
        if (in2 && it[in2].ID==IID_AREA25_TELEKEY) {
            switch (it[in2].drdata[0]) {
                ...
                case 2:		if (ch[cn].saves<10 && !(ch[cn].flags&CF_HARDCORE)) { ch[cn].saves++; log_char(cn,LOG_SYSTEM,0,"You received a save."); }
                    break;
        ...
        if (msg->type==NT_GIVE) {
            ...
            if ((in=ch[cn].citem)) {
                ...
                if (it[in].ID==IID_ALCHEMY_INGREDIENT) {
                    if (type==23) {
                        in2=create_item("warped_door_key"); if (give_char_item(co,in2)) flag=1;
