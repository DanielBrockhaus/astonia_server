...
void show_npc(int cn,int n) {
    ...
    for (i=0; i<5; i++) if (lq_npc[n].trigger[i][0]) log_sys(cn,"Trigger/Reply %d: %s/%s",i,lq_npc[n].trigger[i],lq_npc[n].reply[i]);
    ...
    if (*ptr=='#' || *ptr=='/') {
        ...
        if (ch[cn].flags&(CF_GOD|CF_LQMASTER)) {
            ...
            if ((len=cmdcmp(ptr,"xinfo",2))) {
                for (n=1; n<MAXLQMARK; n++) if (pdat->mark[n]) log_sys(cn,"I have mark %d",n);
...
void lq_ticker(int in,int cn) {
    ...
    for (n=1; n<MAXLQNPC; n++) if (lq_respawn[n] && ticker>=lq_respawn[n] && spawn_npc(n,0,0,0,NULL)) lq_respawn[n]=0;
