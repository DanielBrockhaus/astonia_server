    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_CHAR) {
            ...
            switch (ppd->gatamastate) {
                case 0: if (ppd->salt) ppd->gatamastate=50;
        ...
        if (msg->type==NT_TEXT) {
            ...
            if (strcasestr(str,"REPEAT")) { say(cn,"I will repeat, %s",ch[co].name); ppd->gatamastate=0; } else if (strcasestr(str,"DETAILS")) { ppd->gatamastate=20; } else if (strcasestr(str,"BEGIN")) { ppd->gatamastate=30; }
        }
...
void monk_worker_driver(int cn,int ret,int lastact) {
    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_TEXT) {
            ...
            if (co==workerdat->follow_cn) {
                if (strcasestr(str,"COME")) { workerdat->waituntil=0; } else if (strcasestr(str,"WAIT")) { workerdat->waituntil=ticker+20*TICKS; } else if (strcasestr(str,"SALT")) { log_area(ch[cn].x,ch[cn].y,LOG_SYSTEM,0,10,"%s %s his head.",ch[cn].name,workerdat->hassalt?"nods":"shakes");  }
            }
        }
    ...
    // fighting
    fight_driver_update(cn);
    if (ch[cn].hp>20*POWERSCALE) { if (fight_driver_attack_visible(cn,0)) return; } else { if (fight_driver_flee(cn)) return; }
    if (regenerate_driver(cn)) return;
    if (spell_self_driver(cn)) return;
