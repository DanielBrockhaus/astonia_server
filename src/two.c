...
void guard_parse(int cn,struct twoguard_data *dat) {
    ...
    for (ptr=nextnv(ch[cn].arg,name,value); ptr; ptr=nextnv(ptr,name,value)) {
        ...
        if (!strcmp(name,"patx")) { if (pat>=MAXPAT) elog("twoguard_parse: too many patrol stops");
            else dat->patx[pat]=atoi(value); } else if (!strcmp(name,"paty")) { if (pat>=MAXPAT) elog("twoguard_parse: too many patrol stops");
            else dat->paty[pat++]=atoi(value); } else elog("unknown arg for %s (%d): %s",ch[cn].name,cn,name);
    }
}
...
void guard_driver(int cn,int ret,int lastact) {
    ...
    if (dat->tx) {
        ...
        else if (swap_move_driver(cn,dat->tx,dat->ty,1)) { dat->good_tx_try=ticker; return; } else if (swap_move_driver(cn,dat->tx,dat->ty,3)) { dat->good_tx_try=ticker; return; } else if (ticker-dat->good_tx_try>TICKS*10) { dat->tx=dat->ty=0; }
    }
    ...
    if (ch[cn].tmpx-ch[cn].x==0 && ch[cn].tmpy-ch[cn].y==0) { do_idle(cn,TICKS); } else if (swap_move_driver(cn,ch[cn].tmpx,ch[cn].tmpy,0)) return;
    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_CHAR) {
            ...
            if (ppd) {
                ...
                if (illegal_place(ch[cn].x,ch[co].x)>ppd->citizen_status) { } else {
                    switch (dat->current_state) {
