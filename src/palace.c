...
void palace_guard_parse(int cn,struct palace_guard_data *dat) {
    ...
    for (ptr=nextnv(ch[cn].arg,name,value); ptr; ptr=nextnv(ptr,name,value)) {
        ...
        if (!strcmp(name,"patrolx")) { if (pat>=MAXPAT) elog("palace_guard_parse: too many patrol stops");
            else dat->patrolx[pat]=atoi(value); } else if (!strcmp(name,"patroly")) { if (pat>=MAXPAT) elog("palace_guard_parse: too many patrol stops");
            else dat->patroly[pat++]=atoi(value); } else if (!strcmp(name,"patrol")) dat->patrol=atoi(value);
