...
static void ef_fireball(int fn) {
    ...
    if (abs(dx)>abs(dy)) { dy=dy*512/abs(dx); dx=dx*512/abs(dx); } else { dx=dx*512/abs(dy); dy=dy*512/abs(dy); }
...
static void ef_ball(int fn) {
    ...
    if (abs(dx)>abs(dy)) { dy=dy*128/abs(dx); dx=dx*128/abs(dx); } else { dx=dx*128/abs(dy); dy=dy*128/abs(dy); }
