...
int found_clan(char *name,int cn,int *pclan) {
    ...
    for (n=1; n<MAXCLAN; n++) if (!clan[n].name[0]) break;
...
static int update_treasure(void) {
    ...
    for (cnr=1; cnr<MAXCLAN; cnr++) {
        ...
        if (clan[cnr].treasure.debt>=1000 && clan[cnr].treasure.jewels>0) {
            ...
            if (n>clan[cnr].treasure.jewels) { n=clan[cnr].treasure.jewels; clan[cnr].treasure.jewels=0; } else clan[cnr].treasure.jewels-=n;
