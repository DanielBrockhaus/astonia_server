...
void lab3_passguard_driver(int cn,int ret,int lastact) {
    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_TEXT) {
            ...
            if (strcasestr(str,password)) { say(cn,"Fine, %s, I will open the door for thee. Mayest thou pass the last gate.",ch[co].name); ppd->guard_talkstep=20; } else if (strcasestr(str,"BLUB")) { say(cn,"What?"); dat->last_talk=ticker+10*TICKS;} else if (strcasestr(str,"REPEAT")) { say(cn,"I'll repeat."); ppd->guard_talkstep=0; }
...
void lab3_prisoner_driver(int cn,int ret,int lastact) {
    ...
    for (msg=ch[cn].msg; msg; msg=next) {
        ...
        if (msg->type==NT_TEXT) {
            ...
            if (strcasestr(str,"BLUB")) { say(cn,"Blub!"); ppd->prisoner_talkstep=0; dat->next_talk=ticker+TICKS*5; } else if (strcasestr(str,"REPEAT")) { say(cn,"Blub."); ppd->prisoner_talkstep=0; dat->next_talk=ticker+TICKS*1; }
        }
