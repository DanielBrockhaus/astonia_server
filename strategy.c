    ...
    for (n=0; n<MAX_STR_AREA; n++) {
        ...
        for (ai=pl=m=0; m<area[n].max_spawn; m++) {
            ...
            if (*(unsigned int *)(it[in].drdata+0) && *(unsigned int *)(it[in].drdata+0)!=0xfffff000) {
                ...
                if (*(unsigned int *)(it[in].drdata+0)<0xfffff000) { pl++; winner=*(unsigned int *)(it[in].drdata+0); } else ai++;
            }
        }
        if (pl==1 && ai==0) {
    ...
    if (ad->panic) {
        ...
        for (n=0; n<ad->max_ap; n++) {
            if ((ad->ap[n].dist<=mindist && ad->ap[n].threatcount)) {
                ...
                if (m==ad->max_at) {
                    for (m=0; m<ad->max_at; m++) if (ad->at[m].place==0) break;
                    ad->at[m].place=n;
                    ad->at[m].ticker=ticker;
                    ad->at[m].count=ad->ap[n].threatcount+ad->ap[n].threatncount; //max(ad->at[m].count,ad->ap[n].threatcount+ad->ap[n].threatncount);
                    ad->at[m].level=max(ad->ap[n].threatlevel,ad->ap[n].threatnlevel); //max(max(ad->at[m].level,ad->ap[n].threatlevel),ad->ap[n].threatnlevel);
        ...
        switch (ad->an[n].task) {
            ...
            case T_EGUARD:		if (ad->an[n].target==0) {
                    if (ch[ad->an[n].cn].level<ad->ppd.max_level &&
                        (ad->ap[0].platin>NPCPRICE*2 || ad->free_workers || ad->an[n].platin>ad->ppd.trainspeed*TRAINMULTI*2 || ad->npc_cnt>=ad->ppd.max_worker)) { task_train(n); } else { task_idle(n); }
                } else task_guard(n);
                break;
            case T_IGNORE:		break;
...
int str_exp_cost(struct strategy_ppd *ppd,int nr) {
    switch (nr) {
        case 1:		if (ppd->income<20) return 25;
            else return 0;
        case 2:		if (ppd->max_level<115) return 4;
            else return 0;
        case 3:		if (ppd->max_worker<16) return 10;
            else return 0;
        case 4:		if (ppd->trainspeed<8) return 4;
            else return 0;
        case 5:		if (ppd->warcry<115) return 4;
            else return 0;
        case 6:		if (ppd->endurance<115) return 4;
            else return 0;
        case 7:		if (ppd->speed<115) return 6;
            else return 0;
        case 8:		if (ppd->eguardlvl<115) return 3;
            else return 0;
...
int str_increment(struct strategy_ppd *ppd,int nr) {
    switch (nr) {
        case 1:		if (str_exp_cost(ppd,nr)) return 1;
            else return 0;
        case 2:		if (str_exp_cost(ppd,nr)) return 2;
            else return 0;
        case 3:		if (str_exp_cost(ppd,nr)) return 1;
            else return 0;
        case 4:		if (str_exp_cost(ppd,nr)) return 1;
            else return 0;
        case 5:		if (str_exp_cost(ppd,nr)) return 5;
            else return 0;
        case 6:		if (str_exp_cost(ppd,nr)) return 5;
            else return 0;
        case 7:		if (str_exp_cost(ppd,nr)) return 5;
            else return 0;
        case 8:		if (str_exp_cost(ppd,nr)) return 1;
            else return 0;
        default:	return 0;
    }
}
