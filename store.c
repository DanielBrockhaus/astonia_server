...
static void add_item_to_store(int s,int in) {
    ...
    for (n=0; n<STORESIZE; n++) if (store[s]->ware[n].cnt && objcmp(&store[s]->ware[n].item,&it[in])) break;
    if (n==STORESIZE) { // no such item in stock yet
        for (n=0; n<STORESIZE; n++) if (!store[s]->ware[n].cnt) break;
...
int create_store(int cn,int ignore,int pricemulti) {
    ...
    for (s=1; s<MAXSTORE; s++) if (!store[s]) break;
