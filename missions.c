...
int build_fighter(int x,int y,int diff,int keyID,char *keyname,char *name,char *temp,char *desc,int fID,int sprite,int hasspecialitem,long long extra_flags) {
    ...
    if (hasspecialitem) {
        ...
        if (ch[cn].level<10) { str=3; base=1; } else if (ch[cn].level<17) { str=4; base=10; } else if (ch[cn].level<24) { str=6; base=20; } else if (ch[cn].level<31) { str=7; base=30; } else if (ch[cn].level<38) { str=8; base=40; } else if (ch[cn].level<45) { str=10; base=50; } else if (ch[cn].level<52) { str=12; base=60; } else if (ch[cn].level<60) { str=14; base=70; } else if (ch[cn].level<66) { str=16; base=80; } else if (ch[cn].level<74) { str=18; base=90; } else { str=20; base=90; }
...
void mission_giver_driver(int cn,int ret,int lastact) {
    ...
    if (ticker>dat->next_spec || !ch[cn].item[30]) {
        ...
        if (lvl<10) { str=3; base=1; } else if (lvl<17) { str=4; base=10; } else if (lvl<24) { str=6; base=20; } else if (lvl<31) { str=7; base=30; } else if (lvl<38) { str=8; base=40; } else if (lvl<45) { str=10; base=50; } else if (lvl<52) { str=12; base=60; } else if (lvl<60) { str=14; base=70; } else if (lvl<66) { str=16; base=80; } else if (lvl<74) { str=18; base=90; } else { str=20; base=90; }

        in=create_special_item(str,base,1,50);

        lvl=it[in].min_level;
        if (lvl<10) { cost=500;} else if (lvl<17) { cost=2000/2;} else if (lvl<24) { cost=4000/2;} else if (lvl<31) { cost=6000/2;} else if (lvl<38) { cost=8000/2;} else if (lvl<45) { cost=10000/2;} else if (lvl<52) { cost=12000/2;} else if (lvl<60) { cost=14000/2;} else if (lvl<66) { cost=16000/2;} else if (lvl<74) { cost=18000/2; } else { cost=20000/2; }
            ...
            if (ch[co].flags&CF_PLAYER) {
                ...
                switch ((didsay=analyse_text_driver(cn,msg->dat1,(char *)msg->dat2,co))) {
                    ...
                    case 3:		if (ch[co].flags&CF_GOD) { say(cn,"reset done"); bzero(ppd,sizeof(struct mission_ppd)); dat->last_talk=0; }
                        break;
                    case 4:		if (!ppd->active && !ppd->solved) offer_mission_sub(cn,co,0,ppd);
                        else quiet_say(cn,"You still have a job. Do you want to °c4fail°c0 it?");
                        break;
                    case 5:		if (!ppd->active && !ppd->solved) offer_mission_sub(cn,co,1,ppd);
                        else quiet_say(cn,"You still have a job. Do you want to °c4fail°c0 it?");
                        break;
                    case 6:		if (!ppd->active && !ppd->solved) offer_mission_sub(cn,co,2,ppd);
                        else quiet_say(cn,"You still have a job. Do you want to °c4fail°c0 it?");
                        break;
                    case 7:		if (ppd->sm[0].type && !ppd->active && !ppd->solved) start_mission(cn,co,0,ppd);
                        else quiet_say(cn,"I haven't offered you that job yet.");
                        break;
                    case 8:		if (ppd->sm[1].type && !ppd->active && !ppd->solved) start_mission(cn,co,1,ppd);
                        else quiet_say(cn,"I haven't offered you that job yet.");
                        break;
                    case 9:		if (ppd->sm[2].type && !ppd->active && !ppd->solved) start_mission(cn,co,2,ppd);
                        else quiet_say(cn,"I haven't offered you that job yet.");
                        break;
                    ...
                    case 13:	ppd->dif_kill=max(0,ppd->dif_kill-10);
                        ...
                        break;
                    case 14:	if (ch[co].flags&CF_GOD) { say(cn,"I hate freeloaders!"); ppd->points+=100; }
                        break;
                    case 15:	if (ch[co].flags&CF_GOD) { say(cn,"I hate freeloaders!"); ppd->points+=1000; }
                        break;
                    case 16:	if (ch[co].flags&CF_GOD) { say(cn,"I hate freeloaders!"); ppd->points+=10000; }
                        break;
                    case 17:	if (ch[co].flags&CF_GOD) { say(cn,"I hate freeloaders!"); ppd->points+=100000; }
                        break;
                    case 18:	look_item(co,it+ch[cn].item[30],-1);
